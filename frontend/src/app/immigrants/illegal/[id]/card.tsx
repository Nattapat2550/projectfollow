...
export default function IllegalIDPageCard({ data }: { data: IllegalData | null }) {
	if (!data) return null;
	...
	// วันที่พบตัว / ส่งกลับ
	const dateValue = formatDate(data.detected_date);

	// วันเดือนปีเกิด
	const getDobText = () => {
		return data.date_of_birth ? formatDate(data.date_of_birth) : "-";
	};
				...
				<div className="flex min-w-0 flex-col" style={{ width: "67%", marginRight: "3%" }}>
					...
					<div className="flex w-full justify-between" style={{ marginBottom: "2%" }}>
						<div className="flex flex-col" style={{ width: "48.5%" }}>
							...
							<IBox mono>{data.national_id ? formatNationalId(data.national_id) : "-"}</IBox>
						</div>
						<div className="flex flex-col" style={{ width: "48.5%" }}>
					...
					<div className="flex flex-col" style={{ marginBottom: "2%" }}>
						<ILabel>{"สถานที่ทำงาน / จุดตรวจพบ"}</ILabel>

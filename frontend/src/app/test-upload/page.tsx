      ...
      {result && (
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="p-6 border rounded-xl bg-[var(--container)] shadow-sm border-[var(--wrapper)]">
             ...
             <p className="text-sm mb-4 text-[var(--header)] opacity-80">ตรวจสอบข้อมูลพรีวิวด้านล่าง หากถูกต้องแล้ว กดปุ่มเพื่อนำเข้าฐานข้อมูลจริง</p>
             {isUploading ? (
                      ...
                      <td className="p-4 border-r border-[var(--wrapper)] bg-[var(--container)] opacity-95 align-top text-[var(--header)]">
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-3 pb-3 border-b border-[var(--wrapper)]">
                          <div>
                            ...
                            <div className="text-sm">
                              <span className="text-[var(--header)] opacity-50 mr-2">[DB: middle_name_th]</span>
                              <span className="font-medium text-[var(--blueText)]">{row.middle_name_th || renderNull()}</span>
                            </div>
                            <div className="text-sm">
                          ...
                          <div>
                            ...
                            <div className="text-sm">
                              <span className="text-[var(--header)] opacity-50 mr-2">[DB: middle_name_en]</span>
                              <span className="font-medium text-[var(--blueText)]">{row.middle_name_en || renderNull()}</span>
                            </div>
                            <div className="text-sm">
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-3 pb-3 border-b border-[var(--wrapper)]">
                          <div>
                            <span className="text-[var(--header)] opacity-60 text-xs block">[DB: nationality] สัญชาติ:</span>
                            <span className="font-medium">{row.nationality || renderNull()}</span>
                          </div>
                          <div>
                            <span className="text-[var(--header)] opacity-60 text-xs block">[DB: passport_id] พาสปอร์ต:</span>
                            <span className="font-mono font-medium text-[var(--blueText)]">{row.passport_id || renderNull()}</span>
                          </div>
                          <div>
                            <span className="text-[var(--header)] opacity-60 text-xs block">[DB: gender] เพศ (อัตโนมัติ):</span>
                          ...
                          <div>
                            ...
                            <span className="font-medium bg-[var(--yellowBG)] px-1.5 rounded text-[var(--yellowText)] border border-[var(--yellowBorder)]">
                              {row.detected_date || renderNull()}
                            </span>
                          </div>
                        </div>

                        <div className="grid grid-cols-1 gap-2 mb-3 pb-3 border-b border-[var(--wrapper)]">
                          <div>
                            <span className="text-[var(--header)] opacity-60 text-xs block">[DB: detected_location] สถานที่ตรวจพบ:</span>
                            <span className="font-medium">{row.detected_location || renderNull()}</span>
                          </div>
                          <div className="bg-[var(--container)] p-2 rounded-md border border-[var(--wrapper)]">
                            <span className="text-[var(--blueText)] text-xs font-semibold block">[DB: workplace] สถานที่ทำงาน:</span>
                            <span className="font-medium text-[var(--header)]">{row.workplace || renderNull()}</span>
                          </div>
                        </div>

                        <div className="bg-[var(--container)] rounded-lg p-3 border border-[var(--wrapper)]">
                          <span className="text-[10px] font-semibold text-[var(--header)] opacity-50 uppercase tracking-wider block mb-2">หมวดหมู่ผลการคัดกรอง</span>
                          
                          <div className="mb-2">
                            <span className="text-[var(--header)] opacity-60 text-xs mr-2">[DB: is_victim] Status:</span>
                            {row.is_victim ? (
                              <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-[var(--greenBG)] text-[var(--greenText)] border border-[var(--greenBorder)]">
                                TRUE (เป็นผู้เสียหาย)
                              </span>
                            ) : (
                              <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-[var(--wrapper)] text-[var(--header)]">
                          ...
                          <div>
                            <span className="text-[var(--blueText)] text-xs font-semibold block mb-1">
                              [DB: screening_details] รายละเอียดผลคัดกรอง:
                            </span>
                            <div className="text-sm font-medium text-[var(--header)] bg-[var(--button)] p-2 border border-[var(--wrapper)] rounded-md min-h-10 whitespace-pre-wrap">

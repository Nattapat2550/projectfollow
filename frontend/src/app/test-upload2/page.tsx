      ...
      {result && (
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="p-6 border rounded-xl bg-blue-50 dark:bg-blue-900/20 shadow-sm border-blue-100 dark:border-blue-800">
             ...
             <p className="text-sm mb-4 text-blue-600 dark:text-blue-400">เมื่อกดปุ่มนี้ ระบบจะเริ่มบันทึกข้อมูลและอัปโหลดรูปภาพทั้งหมดขึ้น Google Drive ทันที</p>
             {isUploading ? (
                      ...
                      <td className="p-4 border-r border-zinc-200 dark:border-zinc-800 bg-blue-50/10 dark:bg-blue-950/5 align-top text-zinc-800 dark:text-zinc-200">
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-3 pb-3 border-b border-zinc-200 dark:border-zinc-800">
                          ...
                          <div>
                            ...
                            <div className="text-sm"><span className="text-zinc-500 mr-1">[DB: national_id]</span> <span className="font-mono">{row.id_card || renderNull()}</span></div>
                            <div className="text-sm"><span className="text-zinc-500 mr-1">[DB: passport_id]</span> <span className="font-mono">{row.passport || renderNull()}</span></div>
                            <div className="text-sm mt-1">
                              <span className="text-zinc-500 block mb-2">[DB: photo_url]</span> 
                              {row.photo_url && (row.photo_url.startsWith('data:image') || row.photo_url.startsWith('http')) ? (
                                <img 
                                  src={row.photo_url} 
                                  alt="พรีวิวรูปโปรไฟล์" 
                                  className="w-16 h-20 object-cover rounded-md border border-zinc-200 dark:border-zinc-700 shadow-sm" 
                                />
                              ) : (
                                <span className="font-medium truncate block text-amber-600 dark:text-amber-400" title={row.photo_url}>

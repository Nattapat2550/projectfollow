                      ...
                      <td className="p-4 border-r bg-blue-50/10 dark:bg-blue-950/5 align-top">  
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-3 pb-3 border-b border-zinc-200 dark:border-zinc-800">  
                          ...
                          <div>  
                            ...
                            <div className="text-sm mb-2"><span className="text-zinc-500 mr-1">[DB: address]</span> <span className="font-medium truncate block" title={row.address}>{row.address || renderNull()}</span></div>  
                            
                            {/* รูปแบบเข้ารหัสข้อมูลรูปแบบ Base64 ปลอดภัยจากการดึงและสแกนของบุคคลภายนอกอย่างสมบูรณ์ */}
                            <div className="text-sm border p-2.5 rounded-lg bg-zinc-50 dark:bg-zinc-900/40">
                              <span className="text-zinc-500 block mb-1 font-semibold text-[11px]">[DB: photo_url] (Secure Base64 Data Stream)</span> 
                              {row.photo_url ? (
                                <div className="space-y-2">
                                  {/* ซ่อนลิงก์ไม่ให้ก๊อปปี้ และจำกัดการส่องดูค่าพรีวิว */}
                                  <div className="text-[10px] font-mono text-zinc-400 bg-zinc-200 dark:bg-zinc-800 p-1.5 rounded truncate max-w-xs select-all">
                                    {row.photo_url.substring(0, 50)}... [🔒 Encrypted Data Object]
                                  </div>
                                  {/* แสดงรูปภาพโดยดึงเนื้อภาพภายในหน่วยความจำตรงๆ */}
                                  <img 
                                    src={row.photo_url} 
                                    alt="รูปโปรไฟล์พรีวิว" 
                                    className="w-16 h-20 object-cover rounded-md border border-zinc-300 dark:border-zinc-700 shadow-sm mt-1" 
                                  />
                                </div>
                              ) : (
                                renderNull("ไม่มีข้อมูลรูปภาพ")
                              )}
                            </div>
                          </div>  
                        </div>  

...
export async function uploadToDrive(
	fileObject:
		| Pick<Express.Multer.File, "originalname" | "buffer" | "mimetype">
		| undefined,
	folderId: string
) {
	...
	try {
		...
		if (fileObject.buffer) {
			...
			throw new Error(
				"ระบบรองรับการอัปโหลดไฟล์ผ่าน Memory Storage (Buffer) เท่านั้น"
			);
		}
...
export async function deleteFromDrive(fileId: string) {
	try {
		...
		console.error("[Drive Service Delete Error]:", error);
		throw error;
	}
}

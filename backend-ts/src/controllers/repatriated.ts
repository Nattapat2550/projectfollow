...
export const getRepatriatedById: RequestHandler = async (req, res) => {
	try {
...
export const createRepatriated: RequestHandler = async (req, res) => {
	try {
		const data = req.body;
		...
		if (data.national_id) {
			...
			if (existingNat.rows.length > 0)
				return res.status(400).json({
					success: false,
					message: "เลขประจำตัว (national_id) นี้มีอยู่ในระบบแล้ว",
				});
		}
		...
		if (data.passport_id) {
			...
			if (existingPass.rows.length > 0)
				return res.status(400).json({
					success: false,
					message: "เลขหนังสือเดินทาง (passport_id) นี้มีอยู่ในระบบแล้ว",
				});
		}
...
export const updateRepatriated: RequestHandler = async (req, res) => {
	try {
		const { id } = req.params;
		const data = req.body;
...
export const deleteRepatriated: RequestHandler = async (req, res) => {
	try {
		const { id } = req.params;
		const existingDataRes = await pool.query(

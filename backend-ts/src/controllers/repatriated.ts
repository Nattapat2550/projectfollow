...
import type { CreateRepatriatedRequest } from "@/schema/repatriated";
import type { GetRepatriatedByIdResponse } from "@/schema/repatriated";
import type {
...
export async function getRepatriatedByIdController(
	id: string
): Promise<GetRepatriatedByIdResponse> {
	const { rows } = await pool.query(
		"SELECT t.*, u.name AS creator_name, u.color AS creator_color FROM repatriated_persons t LEFT JOIN users u ON t.created_by = u.id WHERE t.id = $1",
		[id]
	);
...
export async function createRepatriatedController(
	data: CreateRepatriatedRequest,
	files: Request["files"],
	user?: User
): Promise<CreateRepatriatedResponse> {
	...
	if (data.national_id) {
		...
		if (existingNat.rows.length > 0)
			error(400, "เลขประจำตัว (national_id) นี้มีอยู่ในระบบแล้ว");
	}
	...
	if (data.passport_id) {
		...
		if (existingPass.rows.length > 0)
			error(400, "เลขหนังสือเดินทาง (passport_id) นี้มีอยู่ในระบบแล้ว");
	}
...
export async function updateRepatriatedController(
	id: string,
	data: UpdateRepatriatedRequest,
	files: Request["files"]
): Promise<UpdateRepatriatedResponse> {
...
export async function deleteRepatriatedController(
	id: string
): Promise<DeleteRepatriatedResponse> {
	const existingDataRes = await pool.query(

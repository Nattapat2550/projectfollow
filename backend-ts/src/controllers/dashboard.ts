...
export const getDashboardStats: RequestHandler = async (req, res) => {
	try {
		...
		const dateField = type === "repatriated" ? "return_date" : "detected_date";
		...
		if (vStart && vEnd) {
			conditions.push(
				`DATE(t.${dateField}) >= $${paramIndex} AND DATE(t.${dateField}) <= $${paramIndex + 1}`
			);
		...
		if (type === "repatriated") {
			if (vDobStart && vDobEnd) {
				conditions.push(
					`DATE(t.date_of_birth) >= $${paramIndex} AND DATE(t.date_of_birth) <= $${paramIndex + 1}`
				);
    ...
    `;
		...
		charts.gender = genderChartRes.rows.map((r) => ({
			name: r.name,
      ...
      `;
			...
			charts.victim = victimChartRes.rows.map((r) => ({
				name: r.name,
				value: parseInt(r.value),
			}));
			charts.passport = passportChartRes.rows.map((r) => ({
				name: r.name,
				value: parseInt(r.value),
			}));
		} else {
      ...
      `;
			...
			stats.victims = parseInt(victimRes.rows[0].count);
			charts.channel = channelChartRes.rows.map((r) => ({
				name: r.name,
				value: parseInt(r.value),
			}));
			charts.victim = victimChartRes.rows.map((r) => ({
		...
		charts.nationality = natChartRes.rows.map((r) => ({
			name: r.name,
			value: parseInt(r.value),
		}));
    ...
    `;
		...
		charts.province = provinceChartRes.rows.map((r) => ({
			name: r.name,
			value: parseInt(r.value),
		}));
    ...
    `;
		...
		charts.ageGroup = ageChartRes.rows.map((r) => ({
			name: r.name,
			value: parseInt(r.value),
		}));
    ...
    `;
		...
		charts.dateTrend = dateTrendRes.rows.map((r) => ({
			name: r.name,
			value: parseInt(r.value),
		}));
    ...
    `);
		...
		const allCreatorsRes = await pool.query(
			`SELECT DISTINCT u.name as creator FROM ${tableName} t JOIN users u ON t.created_by = u.id WHERE u.name IS NOT NULL ORDER BY u.name`
		);

		const provField =
			type === "repatriated" ? "province" : "detected_location_province";
		const allProvincesRes = await pool.query(
